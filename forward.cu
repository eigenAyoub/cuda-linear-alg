...
int main(){
    ...
    std::cout << "we here " << blockDim.x << "\n";
    std::cout << "we here " << blockDim.y << "\n";
    ...
    softmax<<<gridDims1, blockDims1>>>(smx, Z1_d, HIDDEN_DIM); //  this is acually logsoftmax
    logloss<<<2, 32>>>(L, smx, y_train_d, HIDDEN_DIM);  // we just pick the true label
    rLoss<<<2, 32>>>(l, L);

    std::vector<float> ll(1);
    cudaMemcpy(ll.data(), l, sizeof(float), cudaMemcpyDeviceToHost);

    std::cout << "forward loss = " << ll[0] << "\n";
